...
NOTTINGHAM_MELODY_RANGE = NOTTINGHAM_MELODY_MAX - NOTTINGHAM_MELODY_MIN + 2
CHORD_BASE = 48
...
def combine(melody, harmony):
    ...
    DOCSTRING
    """
    full = numpy.zeros((melody.shape[0], NOTTINGHAM_MELODY_RANGE + num_chords))
...
def parse_nottingham_directory(input_dir, time_step, verbose=False):
    ...
    returns a list of [T x D] matrices,
    where each matrix represents a sequence with T time steps over D dimensions.
    """
    files = [
        os.path.join(input_dir, f) for f in os.listdir(input_dir)
        if os.path.isfile(os.path.join(input_dir, f))] 
    sequences = [
        parse_nottingham_to_sequence(
            f, time_step=time_step, verbose=verbose) for f in files]
    if verbose:
...
def parse_nottingham_to_sequence(input_filename, time_step, verbose=False):
    ...
    returns a [T x D] matrix representing a sequence with T time steps over D dimensions
    """
    sequence = []
    ...
    for i in range(harmony_sequence.shape[0]):
        ...
        if len(notes) > 0:
            ...
            if len(chord) == 0:
                notes_shift = [
                    SHARPS_TO_FLATS[n] if n in SHARPS_TO_FLATS else n for n in notes_shift]
                chord = mingus.core.chords.determine(notes_shift, shorthand=True)
            if len(chord) == 0:
                if verbose:
                    print(
                        "Could not determine chord: {} ({}, {}), defaulting to last steps chord"
...
def prepare_nottingham_pickle(time_step, chord_cutoff=64, filename=PICKLE_LOC, verbose=False):
    ...
    chord_cutoff: if chords are seen less than this cutoff,
        they are ignored and marked as rests in the resulting dataset.
    filename: the location where the pickle will be saved to
    """
    data, store, chords, max_seq, seq_lens = {}, {}, {}, 0, []
    for d in ["train", "test", "valid"]:
        print "Parsing {}...".format(d)
        parsed = parse_nottingham_directory(
            "data/Nottingham/{}".format(d), time_step, verbose=False)
...
def resolve_chord(chord):
    """
    Resolves rare chords to their closest common chord,
    to limit the total amount of chord classes.
    """
...
class NottinghamSampler:
    ...
    def sample_notes_dist(self, probs):
        ...
        ps = numpy.array([n[1] for n in idxed])
        r = NOTTINGHAM_MELODY_RANGE
        assert numpy.allclose(numpy.sum(ps[:r]), 1.0)
        assert numpy.allclose(numpy.sum(ps[r:]), 1.0)
        ps[:r] = ps[:r] / ps[:r].sum()
        ps[r:] = ps[r:] / ps[r:].sum()
        melody = numpy.random.choice(notes[:r], p=ps[:r])
        harmony = numpy.random.choice(notes[r:], p=ps[r:])
    ...
    def visualize_probs(self, probs):
        ...
        if not self.verbose:
            return
        melodies = sorted(list(enumerate(
            probs[:NOTTINGHAM_MELODY_RANGE])), key=lambda x: x[1], reverse=True)[:4]
        harmonies = sorted(list(enumerate(
            probs[NOTTINGHAM_MELODY_RANGE:])),  key=lambda x: x[1], reverse=True)[:4]
        harmonies = [(self.idx_to_chord[i], j) for i, j in harmonies]
        print('Top Melody Notes: ')
        pprint.pprint(melodies)
        ...
        for ts_targets, ts_probs in zip(batch_targets, batch_probs[num_time_steps]):
            ...
            for seq_idx in range(ts_targets.shape[1]):
                for step_idx in range(ts_targets.shape[0]):
                    idxed = [(n, p) for n, p in enumerate(ts_probs[step_idx, seq_idx, :])]
                    notes = [n[0] for n in idxed]
                    ps = numpy.array([n[1] for n in idxed])
                    r = NOTTINGHAM_MELODY_RANGE
                    assert numpy.allclose(numpy.sum(ps[:r]), 1.0)
        ...
        for ts_targets, ts_probs in zip(batch_targets, batch_probs[num_time_steps]):
            ...
            for seq_idx in range(ts_targets.shape[1]):
                for step_idx in range(ts_targets.shape[0]):
                    ...
                    ps = numpy.array([n[1] for n in idxed])
                    r = NOTTINGHAM_MELODY_RANGE
                    assert numpy.allclose(numpy.sum(ps), 1.0)
                    ps = ps / ps.sum()
                    note = numpy.random.choice(notes, p=ps)
