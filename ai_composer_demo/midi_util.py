...
def round_notes(notes, track_ticks, time_step, R=None, O=None):
    ...
    if not O:
        O = 0
    sequence = numpy.zeros((track_ticks/time_step, R))
    disputed = {t: collections.defaultdict(int) for t in range(track_ticks/time_step)}
    for note in notes:
    ...
    for seq_idx in range(sequence.shape[0]):
        if numpy.count_nonzero(sequence[seq_idx, :]) == 0 and len(disputed[seq_idx]) > 0:
...
class MidiWriter:
    ...
    def dump_sequence_to_midi(
        ...
        sequence = numpy.reshape(sequence, [-1, self.note_range])
        ...
        for seq_idx in range(time_steps):
            notes = numpy.nonzero(sequence[seq_idx, :])[0].tolist()
            # this tick will only be assigned to first NoteOn/NoteOff in this time_step
            # NoteOffEvents come first so they'll have the tick value
            # go through all notes that are currently on and see if any turned off
