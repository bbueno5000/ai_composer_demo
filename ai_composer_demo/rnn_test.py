...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Script to test a models performance against the test set')
    ...
    with tensorflow.Graph().as_default(), tensorflow.Session() as session:
        ...
        test_loss, test_probs = util.run_epoch(
            session, test_model, test_data, training=False, testing=True)
        print('Testing Loss: {}'.format(test_loss))
        if config.dataset == 'softmax':
            if args.seperate:
                nottingham_util.seperate_accuracy(
                    test_probs, test_data, num_samples=args.num_samples)
            else:
