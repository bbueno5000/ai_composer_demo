...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Script to generated a MIDI file sample from a trained model.')
    ...
    parser.add_argument(
        '--sample_seq', type=str, default='random', choices = ['random', 'chords'])
    ...
    if config.dataset == 'softmax':
        ...
        model_class = model.NottinghamModel
        ...
        test_data = util.batch_data(
            pickle['test'], time_batch_len = 1, max_time_batches = -1, softmax = True)
    else:
    ...
    with tensorflow.Graph().as_default(), tensorflow.Session() as session:
        ...
        model_path = os.path.join(
            os.path.dirname(args.config_file), config.model_name)
        ...
        if args.sample_seq == 'chords':
            ...
            sample_index = numpy.random.choice(numpy.arange(len(pickle['test'])))
            sample_seq = [
                pickle['test'][sample_index][i, :]
                for i in range(pickle['test'][sample_index].shape[0])]
        chord = sample_seq[0]
        seq = [chord]
        if args.conditioning > 0:
            for i in range(1, args.conditioning):
                seq_input = numpy.reshape(chord, [1, 1, config.input_dim])
                feed = {
        ...
        for i in range(max(args.sample_length - len(seq), 0)):
            ...
            [probs, state] = session.run(
                [sampling_model.probs, sampling_model.final_state], feed_dict=feed)
            probs = numpy.reshape(probs, [config.input_dim])
            ...
            if config.dataset == 'softmax':
                ...
                if args.sample_melody:
                    ...
                    chord[:r] = sample_seq[i][:r]
            seq.append(chord)
        writer.dump_sequence_to_midi(
            seq, "best.midi", time_step=time_step, resolution=resolution)
