            ...
            for seq_idx in range(ts_targets.shape[1]):
                for step_idx in range(ts_targets.shape[0]):
                    for note_idx, prob in enumerate(ts_probs[step_idx, seq_idx, :]):
                        num_occurrences = numpy.random.binomial(num_samples, prob)
...
def arrange_batch(sequences, num_time_steps):
    ...
    stacked = numpy.dstack(sequences)
    # swap axes so that shape is (SEQ_LENGTH X BATCH_SIZE X INPUT_DIM)
    data = numpy.swapaxes(stacked, 1, 2)
    targets = numpy.roll(data, -1, axis=0)
    ...
    if softmax:
        ...
        labels = numpy.ones((targets.shape[0], targets.shape[1], 2), dtype=numpy.int32)
        assert numpy.all(numpy.sum(targets[:, :, :r], axis=2) == 1)
        assert numpy.all(numpy.sum(targets[:, :, r:], axis=2) == 1)
        labels[:, :, 0] = numpy.argmax(targets[:, :, :r], axis=2)
        labels[:, :, 1] = numpy.argmax(targets[:, :, r:], axis=2)
        targets = labels
        assert targets.shape[:2] == data.shape[:2]
    assert data.shape[0] == num_time_steps * time_batch_len
    # split them up into time batches
    tb_data = numpy.split(data, num_time_steps, axis=0)
    tb_targets = numpy.split(targets, num_time_steps, axis=0)
    ...
    for i in range(len(tb_data)):
        ...
        if softmax:
            assert numpy.all(numpy.sum(tb_data[i], axis=2) == 2)
...
def load_data(data_dir, time_step, time_batch_len, max_time_batches, nottingham=None):
    ...
    for dataset in ['train', 'test', 'valid']:
        ...
        # Cross-entropy formulation needs to be parsed
            ...
            metadata = [{
                'path': f, 'name': f.split("/")[-1].split(".")[0]} for f in files]
        dataset_data = batch_data(
            sequences, time_batch_len, max_time_batches,
...
def parse_midi_directory(input_dir, time_step):
    ...
    sequences = [
        (f, midi_util.parse_midi_to_sequence(f, time_step=time_step)) for f in files]
    return sequences
...
def run_epoch(session, model, batches, training=False, testing=False):
    ...
    if testing:
        ...
        batch_probs = collections.defaultdict(list)
    if training:
    ...
    for data, targets in batches:
        ...
        for tb_data, tb_targets in zip(data, targets):
            if testing:
                ...
                permutations = numpy.random.permutation(batches)
                tbd = numpy.zeros_like(tb_data)
                tbd[:, numpy.arange(batches), :] = tb_data[:, permutations, :]
                tbt = numpy.zeros_like(tb_targets)
                tbt[:, numpy.arange(batches), :] = tb_targets[:, permutations, :]
                state[numpy.arange(batches)] = state[permutations]
            feed_dict = {
