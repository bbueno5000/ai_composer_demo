...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--choice', type=str, default='melody', choices = ['melody', 'harmony'])
    parser.add_argument(
        '--dataset', type=str, default='softmax', choices = ['bach', 'nottingham', 'softmax'])
    ...
    if args.dataset == 'softmax':
        resolution = 480
        time_step = 120
        model_class = model.NottinghamSeparate
    ...
    for combination in runs:
        config = rnn.DefaultConfig()
        config.dataset = args.dataset
        config.model_name = ''.join(random.choice(
            string.ascii_uppercase + string.digits) for _ in range(12)) + '.model'
        for attr, value in combination:
            setattr(config, attr, value)
        if config.dataset == 'softmax':
            data = util.load_data(
                '', time_step, config.time_batch_len,
                config.max_time_batches, nottingham=pickle)
        ...
        logger.info(config)
        config_file_path = os.path.join(run_folder, rnn.get_config_name(config) + '.config')
        with open(config_file_path, 'w') as f: 
            cPickle.dump(config, f)
        with tensorflow.Graph().as_default(), tensorflow.Session() as session:
            with tensorflow.variable_scope("model", reuse=None):
                train_model = model_class(config, training=True)
            with tensorflow.variable_scope("model", reuse=True):
                valid_model = model_class(config, training=False)
            saver = tensorflow.train.Saver(tensorflow.all_variables())
            ...
            for i in range(config.num_epochs):
                loss = util.run_epoch(
                    session, train_model, data["train"]["data"],
                    training=True, testing=False)
                train_losses.append((i, loss))
                if i == 0:
                    continue
                valid_loss = util.run_epoch(
                    session, valid_model, data["valid"]["data"],
                    training=False, testing=False)
                valid_losses.append((i, valid_loss))
                logger.info(
                    'Epoch: {}, Train Loss: {}, Valid Loss: {}, Time Per Epoch: {}'.format(
            ...
            if config.dataset == 'softmax':
                ...
                axes.set_ylim([0, 100])
            pyplot.plot([t[0] for t in train_losses], [t[1] for t in train_losses])
            pyplot.plot([t[0] for t in valid_losses], [t[1] for t in valid_losses])
            pyplot.legend(['Train Loss', 'Validation Loss'])
            chart_file_path = os.path.join(run_folder, rnn.get_config_name(config) + '.png')
