...
def get_config_name(config):
    ...
    def replace_dot(s):
        return s.replace(".", "p")
    return (
        "nl_" + str(config.num_layers) + "_hs_" + str(config.hidden_size) +
...
def train_model():
    ...
    parser.add_argument(
        '--dataset', type=str, default='softmax', choices = ['softmax'])
    ...
    if os.path.exists(run_folder):
        raise Exception(
            "Run name {} already exists, choose a different one", format(run_folder))
    ...
    for combination in runs:
        ...
        with tensorflow.Graph().as_default(), tensorflow.Session() as session:
            ...
            saver = tensorflow.train.Saver(tensorflow.all_variables(), max_to_keep=40)
            ...
            for i in range(config.num_epochs):
                ...
                if i == 0:
                    continue
                logger.info(
                    'Epoch: {}, Train Loss: {}, Time Per Epoch: {}'.format(
                        i, loss, (time.time() - start_time)/i))
                valid_loss = util.run_epoch(
                    session, valid_model, data["valid"]["data"], training=False, testing=False)
